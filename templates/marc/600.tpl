  ...
  {foreach from=$fieldInstances item=field}
    <span class="600">
      ...
      {foreach from=$field->subfields key=code item=value name=subfields}
        ...
        {if $code == 'a'}
          <a href="{$record->filter('600a_PersonalNameSubject_personalName_ss', $value)}" class="record-link">{include 'data/subfield.tpl' value=$value}</a>{$comma}
        {elseif $code == 'b'}
          numeration: <a href="{$record->filter('600b_PersonalNameSubject_numeration_ss', $value)}" class="record-link">{include 'data/subfield.tpl' value=$value}</a>{$comma}
        {elseif $code == 'c'}
        ...
        {elseif $code == '0'}
          authority: <a href="{$record->filter('6000_PersonalNameSubject_authorityRecordControlNumber_ss', $value)}" class="record-link">{include 'data/subfield.tpl' value=$value}</a>{$comma}
        {/if}
      {/foreach}
    </span>
    <br/>
