    ...
    {if $recordCount > 100}
      {foreach from=$pages item=page}
        <a class="clickMore clickMore-{$categoryId}-{$typeId}"
           ...
           href="?tab=issues&ajax=1&action=ajaxIssue&categoryId={$categoryId}&typeId={$typeId}{if !is_null({$path})}&path={$path}{/if}&page={$page}">[{$page+1}]</a>
      {/foreach}
      {if $listType == 'filtered-list'}
        |
        <a class="clickMore clickMore-{$categoryId}-{$typeId}"
           ...
           href="?tab=issues&ajax=1&action=ajaxIssue&categoryId={$categoryId}&typeId={$typeId}">list all</a>
      {/if}
    {else}
      <a class="clickMore clickMore-{$categoryId}-{$typeId}"
         style="font-weight: bold"
