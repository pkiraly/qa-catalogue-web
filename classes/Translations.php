...
class Translations extends BaseTab {

  protected string $action = 'list';
  protected $parameterFile = 'translation-analysis.params.json';
  ...
  private function processListRequest(&$smarty) {
    ...
    $result = readCsv($this->getFilePath('translations-stat.csv'), 'id');
  ...
  public function queryUrl($id, $value) {
    return sprintf('?tab=data&type=translations&query=%s:%s', $id, $value);
  }
  ...
  private function download($ruleId, $value) {
    $groupId = '';
    $attachment = sprintf('attachment; filename="translations-issue-%s-is-%s-at-%s.csv"', $ruleId, $value, date("Y-m-d"));
